...
const AnomalousEffectModal = ({ isOpen, onClose, armor }) => {
  if (!isOpen || !armor) return null;
  ...
  return (
    ...
    >
      <div 
        className={`bg-gradient-to-br ${getRarityColor(armor.rarity)} backdrop-blur-xl border border-white/20 rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl`}
        ...
        <div className="bg-black/20 border-b border-white/10 p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-bold text-white">{armor.name} - Anomalous Effect</h2>
            </div>
            <button
              onClick={onClose}
  ...
  return (
    <>
      <div className="bg-black/20 backdrop-blur-lg border border-white/10 rounded-2xl overflow-hidden transition-all duration-300 hover:bg-black/30 hover:scale-105 hover:shadow-2xl group h-full flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600/30 to-indigo-600/30 border-b border-white/10 p-4 relative">
          <div className="text-center">
            ...
            <div className="text-sm text-blue-300">
              {props.currArmor.equippedTo} Armor
            </div>
          </div>
        ...
        <div className="p-4 space-y-3">
          <div className="flex flex-wrap gap-2 justify-center">
            <div className={`bg-gradient-to-r ${getRarityColor(props.currArmor.rarity)} px-3 py-1 rounded-full text-xs font-bold border`}>
              {getRarityLabel(props.currArmor.rarity)} ({props.currArmor.rarity})
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-blue-600/20 text-blue-300 px-2 py-1 rounded text-xs border border-blue-500/30 text-center">
              <div className="font-bold">{defenses[0]}</div>
              <div className="text-xs opacity-75">Melee Def</div>
            </div>
            <div className="bg-blue-600/20 text-blue-300 px-2 py-1 rounded text-xs border border-blue-500/30 text-center">
              <div className="font-bold">{defenses[1]}</div>
              <div className="text-xs opacity-75">Ranged Def</div>
            </div>
            <div className="bg-purple-600/20 text-purple-300 px-2 py-1 rounded text-xs border border-purple-500/30 text-center">
              <div className="font-bold">{props.currArmor.soak}</div>
              <div className="text-xs opacity-75">Soak</div>
            </div>
            <div className="bg-amber-600/20 text-amber-300 px-2 py-1 rounded text-xs border border-amber-500/30 text-center">
              <div className="font-bold">{props.currArmor.encumbrance}</div>
          ...
          <div className="flex flex-col gap-2">
            ...
            {isAdmin && (
              ...
              >
                ...
                <span>{isHidden ? 'Show' : 'Hide'} Armor</span>
              </button>
            )}
          </div>
        </div>
